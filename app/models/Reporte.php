...
class Reporte
{
    ...
    public function listar_datos_egresos($dato){
        try{
            $sql = 'select SUM(egreso_monto) total from movimientos where date(egreso_fecha_registro) = ? and movimiento_tipo = 2 and egreso_estado = 1';
    ...
    public function listar_datos_ingresos_caja($dato){
        try{
            $sql = 'select SUM(egreso_monto) total from movimientos where date(egreso_fecha_registro) = ? and movimiento_tipo = 1  and egreso_estado = 1';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$dato]);
            $return = $stm->fetch();
        } catch (Throwable $e){
    ...
    public function listar_datos_ingresos($dato){
        try{
    ...
    public function sumar_caja($dato){
        try{
    ...
    public function reporte_productos($fecha_filtro,$fecha_filtro_fin){
        try{
            $sql = 'select sum(vd.venta_detalle_cantidad) total, p.producto_nombre from ventas v inner join ventas_detalle vd on v.id_venta = vd.id_venta 
                    ...
                    productos p on cd.id_producto = p.id_producto where date(venta_fecha) between ? and ? group by p.id_producto ';
            $stm = $this->pdo->prepare($sql);
